...
export default {
    ...
    computed: {
        ...
        discountedPrice() {
            ...
            if (this.upsell.amount_type == "percentage") {
                price = Number(
                    (Number(price) * (100 - this.upsell.amount)) / 100
                ).toFixed(2);
        ...
        noConversionDiscontedPrice() {
            ...
            if (this.upsell.amount_type == "percentage") {
                price = Number(
                    (price * (100 - this.upsell.amount)) / 100
                ).toFixed(2);
            } else if (this.upsell.amount_type == "fixed_amount") {
                price = Number(price - this.upsell.amount).toFixed(2);
            }
    ...
    methods: {
        ...
        updateVariantToCart(variant) {
            var variantIds = this.product.variants.map(v => v.id);
            var updates = "";
